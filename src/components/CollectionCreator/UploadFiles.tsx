...
const uploadImageData = async (file: File) => {
  ...
  return (
    <div className="w-full lg:w-4/5 md:text-left">
        <div className="grid grid-cols-1 md:grid-cols-2 mt-10 items-center gap-20">
        <div className="flex flex-col items-center order-1 w-fit p-2 bg-[#2e2c38] rounded-2xl">
            <input
                type="file"
                name="CoverImage"
        ...
        <div className="w-full order-2">
            ...
            { ImageData ? 
            <div className="w-full h-full rounded-2xl text-center flex flex-col items-center border-dotted border-2 border-zinc-100/100">
               <p>{ImageData.name}</p>
            </div>
            : 
    ...
    <button className="bg-gradient-to-r from-fuchsia-500 to-blue-500 p-3 rounded-3xl w-2/5 min-w-[100px] text-center" onClick={submitFormData}>Next</button>
    </div>
  );
}
