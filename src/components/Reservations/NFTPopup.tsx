...
const NFTPopup: React.FC<{showModal: boolean, handleClose: any, collection: string, selected: number}> = ({showModal, handleClose, collection, selected}) => {
  ...
  const onSubmit = () => {
    const val = (document.getElementById(`reserve-input-${reserveMethod}`) as HTMLInputElement).value;
  ...
  return (
    ...
    <div className="relative top-[30%] mx-auto p-5 w-96 h-[300px] shadow-lg rounded-2xl bg-[#2e2c38] text-white flex flex-col items-center justify-center outline-none">
      ...
      <h1 className="text-2xl text-transparent tracking-wide bg-clip-text bg-gradient-to-br from-violet-500 to-fuchsia-500 font-semibold">Reserving: {collection} #{selected}</h1>
      <p>How would you like to reserve this NFT?</p>
      </div>
      <div id="menu-options" className="flex flex-col items-center w-3/5 space-y-2 mt-5">
      ...
      <div id="reserve-input-0" className="hidden mt-5">
        ...
        <input id="input-email" type="email" className="w-full text-zinc-500 text-2xl p-1 rounded-lg bg-white border-2 border-zinc-300 outline-none" onChange={event => console.log(event.target.value)}/>
      </div>
      <div id="reserve-input-1" className="hidden mt-5">
        <label htmlFor="input-address" className="block py-2">Wallet address</label>
        <input id="input-address" type="text" className="w-full text-zinc-500 text-2xl p-1 rounded-lg bg-white border-2 border-zinc-300 outline-none " defaultValue={wallet?.accounts[0].address} onChange={event => console.log(event.target.value)}/>
      </div>
      <div id="reserve-input-2" className="hidden mt-5">
        <label htmlFor="input-ens" className="block py-2">ENS name</label>
        <input id="input-ens" type="text" className="w-full text-zinc-500 text-2xl p-1 rounded-lg bg-white border-2 border-zinc-300 outline-none" onChange={event => console.log(event.target.value)}/>
      </div>
    </div>
    </Modal>
    )
